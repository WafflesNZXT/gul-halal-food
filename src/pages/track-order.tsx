...
export default function TrackOrder() {
  ...
  const findOrder = async (event: React.FormEvent) => {
    ...
    try { setOrder(await lookupOrder(reference.trim().toUpperCase(), contact.trim())); }
    catch (lookupError) {
      const status = (lookupError as { status?: number }).status;
      setError(status === 429 ? "Too many attempts. Please wait a few minutes and try again." : status === 500 || status === 503 ? "Order tracking is temporarily unavailable. Please try again." : status === 403 ? "This request was not accepted. Please try again from the Gul Halal Food website." : "We could not find an order matching those details.");
    } finally { setBusy(false); }
  };
  const forget = () => { forgetLatestOrder(localStorage); setLatest(null); };

  if (order) return <Layout><section className="bg-background px-4 pb-24 pt-36 md:px-6"><div className="mx-auto max-w-3xl"><div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-4xl text-primary">Track Your Order</h1><p className="mt-2 text-lg text-foreground/75">Here is the latest information for your order request.</p></div><button type="button" onClick={() => setOrder(null)} className="min-h-12 rounded-xl border border-primary px-5 text-lg font-bold text-primary">Find another order</button></div><CustomerTrackingActions reference={order.reference} compact /><CustomerOrderStatus order={order} /></div></section></Layout>;
